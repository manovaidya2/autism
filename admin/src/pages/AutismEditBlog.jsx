...
export default function AutismEditBlog() {
  const { id } = useParams();
  ...
  const fetchBlogData = async () => {
    try {
      ...
      // Set editor content after it's rendered
      setTimeout(() => {
  ...
  const handleUpdate = async (e) => {
    ...
    // If image is empty and there was an original image, keep the original
    if (!blogData.image && originalImage) {
      blogData.image = originalImage;
    }
        ...
        <form onSubmit={handleUpdate} className="space-y-6">
          ...
          <div className="flex gap-4">
            <button
              ...
              className={`flex-1 bg-green-600 text-white px-6 py-3 rounded-xl font-semibold hover:bg-green-700 ${
