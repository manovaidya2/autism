// File: AdultBlogForm.jsx
import React, { useRef, useState, useEffect } from "react";
import { useNavigate, useParams } from "react-router-dom";
import axiosInstance from "../api/axiosInstance";
import { FaSave, FaTrash, FaArrowLeft, FaPlus, FaTimes } from "react-icons/fa";

export default function AdultBlogForm() {
  ...
  const loadPost = async () => {
    try {
      const response = await axiosInstance.get(`/adultblog/admin/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditing) {
        posts = posts.map(post => (post._id || post.id) == id ? newPost : post);
      ...
      localStorage.setItem("adult_blog_posts_admin", JSON.stringify(posts));
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-blue-50 py-8 px-4">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-6">
          <button
            onClick={() => navigate("/admin/adult-blog")}
